...
def _inject_obs_from_gdp(u_obs: np.ndarray, v_obs: np.ndarray, obs_mask: np.ndarray, time: np.ndarray, lat: np.ndarray, lon: np.ndarray, gdp_path: str):
...
class SRDataset(Dataset):
    def __init__(self, data_path: str, gdp_path: str | None = None, max_obs_speed: float = 5.0):
        ...
        if "u_bg" in ds and "v_bg" in ds and "u_obs" in ds and "v_obs" in ds:
            ...
            if gdp_path and Path(gdp_path).exists():
                _inject_obs_from_gdp(u_obs, v_obs, np.zeros_like(u_bg, dtype=bool), self.time.to_numpy(dtype="datetime64[ns]"), lat, lon, gdp_path)
    ...
    def __getitem__(self, i):
        ...
        return (
            x,
            torch.from_numpy(self.u_bg[i]),
            torch.from_numpy(self.v_bg[i]),
            torch.from_numpy(self.u_obs[i]),
            torch.from_numpy(self.v_obs[i]),
            torch.from_numpy(self.obs_mask[i]),
        )


class SmallSRMLP(nn.Module):
    def __init__(self, scale=2, ch=64):
        super().__init__()
        self.body = nn.Sequential(
            nn.Conv2d(2, ch, 3, padding=1), nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        return self.ps(self.tail(self.body(x)))
...
def evaluate(model, loader, device, args):
    ...
    for x, low_u, low_v, low_u_obs, low_v_obs, low_mask in loader:
        ...
        if low_mask.any():
            l_obs_u = ((u_lr[low_mask] - low_u_obs[low_mask]) ** 2).mean()
            l_obs_v = ((v_lr[low_mask] - low_v_obs[low_mask]) ** 2).mean()
            l_obs = args.w_obs_u * l_obs_u + args.w_obs_v * l_obs_v
...
def train(args):
    ...
    model = SmallSRMLP(scale=args.scale, ch=args.channels).to(device)
...
def build_parser():
    p = argparse.ArgumentParser(description="Strict PINN-SR MLP training with weighted obs loss (u/v)")
    ...
    p.add_argument("--out-dir", type=str, default=r"D:/data/output/checkpoints_sr_mlp_strict_weighted")
